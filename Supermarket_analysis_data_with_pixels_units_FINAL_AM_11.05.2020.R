...
# Supermarket task - FINAL - suitable for analysis of the data collected BEFORE the update in April 2019 - author: Adriana Michalak - 11.05.2020
...
# ANALYSES: analyses are done for ALL participants; OUTCOMES: outcomes are generated for ALL participants (merged single outcome .csv file)
...
files <-dir(path = "C:\\Users\\adria\\Desktop\\PhD\\Rrr\\Supermarket_AM\\Data\\before_update", recursive = TRUE,include.dirs = TRUE,pattern = "(.*)csv$") # change the directory 

print (files) # print out all file which have been found in a given directory 

for (k in 1:length(files)) # the for loop is going through all the files which were found in a given directory 
{
...
write.csv (outcomes.all.ppt,"C:\\Users\\adria\\Desktop\\PhD\\Rrr\\Supermarket_AM\\Outcomes\\Supermarket_outcomes_1_AM_21.05.2020.csv",row.names=F) # save the merged database under given directory and given name (last bit before .csv)
