...
# Episodic Memory - Sleep deprivation - Adriana Michalak - 04.04.2020
...
setwd("C:\\Users\\adria\\Desktop\\PhD\\Rrr\\COGT\\EpisodicMemory\\Data\\SleepDeprivation") #set directory

getwd()

files <-dir(path = "C:\\Users\\adria\\Desktop\\PhD\\Rrr\\COGT\\EpisodicMemory\\Data\\SleepDeprivation", recursive = TRUE,include.dirs = TRUE,pattern = "^APO(.*)csv$")
print (files)
...
for (k in 1:length(files))
{
  ...
  my_data <- read.csv(files[k], header = FALSE, skip=1) # skip the first row as it is not relevant 
  ...
  names(time_needed_to_complete_the_task)[1] <- "time_needed_tom_complete_the_task_ms"
  ...
  recognition_memory_accuracy <- data.frame('participant_ID'=form[k],'session_number'=form1[k], 'date' = date, 'time'= time, 'protocol_1_SD_2_MN'=1, time_needed_to_complete_the_task, recognition_memory_accuracy)
  ...
  names(recognition_memory_RT)[1] <- "Recognition_Hits_ms"
  ...
  names(source_memory)[2] <- "Source_Memory_Hit_percentage"
  ...
  form = files
  pathOut= ("C:\\Users\\adria\\Desktop\\PhD\\Rrr\\COGT\\EpisodicMemory\\Outcomes\\SleepDeprivation\\")
...
setwd("C:\\Users\\adria\\Desktop\\PhD\\Rrr\\COGT\\EpisodicMemory\\Outcomes\\SleepDeprivation\\") #set directory
#select file names to be merged, i.e., outcomes of the for loop above
srosia <- list.files (path="C:\\Users\\adria\\Desktop\\PhD\\Rrr\\COGT\\EpisodicMemory\\Outcomes\\SleepDeprivation\\", pattern = "APO")
print(srosia)
...
outcomes_all_ppt_Episodic_memory_SleepDeprivation <- do.call (rbind.data.frame, daisy)

write.csv (outcomes_all_ppt_Episodic_memory_SleepDeprivation,"C:\\Users\\adria\\Desktop\\PhD\\Rrr\\COGT\\EpisodicMemory\\Outcomes\\Episodic_Memory_Final_SleepDeprivation_16.12.2020.csv",row.names=F)
