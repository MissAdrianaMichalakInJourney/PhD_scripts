...
# Supermarket task - FINAL - suitable for analysis of the data collected AFTER the update in April 2019 - author: Adriana Michalak - 11.05.2020

# update in April 2019 - map location variables were updated to be in % instead of pixels (datafiles after the updated starts with "supermarket + specified ID")

# ANALYSES: analyses are done for ALL participants (files --> Data); OUTCOMES: outcomes are generated for ALL participants (merged single outcome .csv file)


setwd("C:\\Users\\adria\\Desktop\\PhD\\Rrr\\Supermarket_AM\\Data\\after_update") # set directory

getwd() # get your directory to just double check that directory is the correct one

files <-dir(path = "C:\\Users\\adria\\Desktop\\PhD\\Rrr\\Supermarket_AM\\Data\\after_update", recursive = TRUE,include.dirs = TRUE,pattern = "^supermarket(.*)csv$") # change the directory 
...
for (k in 1:length(files)) # the for loop is going through all the files which were found in the given directory 
{
  ...
  ALLO<- sqrt((my.data$map.location.x - my.data$map.location.correct.x)^2 + (my.data$map.location.y - my.data$map.location.correct.y)^2)
  
  print(ALLO) # outcome variable #3 - print the variable to see the outcomes 
  
  # variable 4 - border effect 
  
  BorderEffect <- {is.logical(my.data$map.width*0.28<my.data$map.location.x+my.data$map.width*0.5&my.data$map.width*0.72>my.data$map.location.x+my.data$map.width*0.5)&(my.data$map.height*0.28<my.data$map.location.y+my.data$map.height*0.5&my.data$map.height*0.72>my.data$map.location.y+my.data$map.height*0.5)}
  ...
  pathOut= ("C:\\Users\\adria\\Desktop\\PhD\\Rrr\\Supermarket_AM\\Outcomes\\after_update\\")
  write.csv(outcomes,paste(pathOut,file = form[k]),row.names=F) # it will generate the .csv with the outcomes of interest for each participants, we will marge them below
  
}

###### marge all the outcomes into one large database 

setwd("C:\\Users\\adria\\Desktop\\PhD\\Rrr\\Supermarket_AM\\Outcomes\\after_update\\") # set new directory

# select file names to be marged, i.e., outcomes from the for loop (look up)

select_files <- list.files (path="C:\\Users\\adria\\Desktop\\PhD\\Rrr\\Supermarket_AM\\Outcomes\\after_update\\", pattern = "outcomes") # search for the files with "outcome" in the name in the given directory
...
write.csv (outcomes.all.ppt,"C:\\Users\\adria\\Desktop\\PhD\\Rrr\\Supermarket_AM\\Outcomes\\Supermarket_outcomes_2_AM_11.05.2020.csv",row.names=F) # save the merged database under given directory and given name (last bit before .csv)
