...
# Sea Hero Quest (SHQ) - FLARE levels - Sleep_deprivation - Adriana Michalak - 15.12.2020
...
for (k in 1:length(files)) {
  ...
  if (check_if_flare[k]==TRUE) {
    ...
    save.name5
    save.name6 <- sub (".*_", "", save.name2) # t session only 
    save.name6
    
    flares <- data.frame ('Participant_ID'= save.name5[k],'protocol_1_SD_2_MN'=1,'session_number'=save.name6[k], 'level_ID'= json_data_frame$meta.level_id,'flare_accuracy'= json_data_frame$meta.flare_accuracy, 
                          'time_needed_to_complete'=json_data_frame$meta.duration)
    flares
    
    form = sprintf('outcomes_%s', save.name)
    pathOut= ("C:\\Users\\adria\\Desktop\\PhD\\Rrr\\SHQ\\Outcomes\\SHQ_SleepDeprivation\\Outcomes_flares\\")
...
setwd("C:\\Users\\adria\\Desktop\\PhD\\Rrr\\SHQ\\Outcomes\\SHQ_SleepDeprivation\\Outcomes_flares\\") # set new directory

# select file names to be marged, i.e., outcomes from the for loop (look up)

select_files <- list.files (path="C:\\Users\\adria\\Desktop\\PhD\\Rrr\\SHQ\\Outcomes\\SHQ_SleepDeprivation\\Outcomes_flares\\", pattern = "outcomes") # search for the files with "outcome" in the name in the given directory
...
write.csv(outcomes.all.ppt,"C:\\Users\\adria\\Desktop\\PhD\\Rrr\\SHQ\\Outcomes\\SHQ_outcomes_flares_SleepDeprivation_AM_15.12.2020.csv",row.names=F) # it will generate the .csv with the outcomes of interest for each participants, we will marge them below
