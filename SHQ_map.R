...
lmer_SHQ_distance_EASY_levels <- lmer(log10(wayfinding_distance)  ~ as.factor(session_number) + age + sex + genotype + protocol + years_of_education + hours_of_sleep_baseline +
                                        protocol*session_number + protocol*genotype + session_number*genotype + session_number*protocol*genotype +(1|id), data=wayfinding_EASY,  na.action = na.exclude)

# two figures - MN and SD --> slide 14
# SD vs MN for e4+ and e4- --> slide 13

distance_easy <- anova (lmer_SHQ_distance_EASY_levels)
write.csv(distance_easy, 'C:\\Users\\adria\\Desktop\\distance_easy.csv')

distance_easy_eta <- eta_sq(lmer_SHQ_distance_EASY_levels)
write.csv(distance_easy_eta, 'C:\\Users\\adria\\Desktop\\distance_easy_eta.csv')
...
lms_SHQ_distance_EASY_levels <- lsmeans(lmer_SHQ_distance_EASY_levels, ~ session_number*genotype*protocol) # plot main effect of: genotype, protocol
...
lmer_SHQ_distance_HARD_levels <- lmer(log10(wayfinding_distance) ~ as.factor(session_number) + age + sex + genotype + protocol + years_of_education + hours_of_sleep_baseline +
                                        protocol*session_number + protocol*genotype + session_number*genotype + session_number*protocol*genotype +(1|id),data=wayfinding_HARD,  na.action = na.exclude)

# two figures - MN and SD --> slide 14
# SD vs MN for e4+ and e4- --> slide 13

distance_hard <- anova (lmer_SHQ_distance_HARD_levels)
write.csv(distance_hard, 'C:\\Users\\adria\\Desktop\\distance_hard.csv')

distance_hard_eta <- eta_sq(lmer_SHQ_distance_HARD_levels)
write.csv(distance_hard_eta, 'C:\\Users\\adria\\Desktop\\distance_hard_eta.csv')
...
lms_SHQ_distance_HARD_levels <- lsmeans(lmer_SHQ_distance_HARD_levels, ~ session_number*genotype*protocol) # plot main effect of: genotype, protocol
...
lmer_SHQ_duration_hard_and_easy_levels <- lmer(log10(wayfinding_duration)  ~ as.factor(session_number) + age + sex + genotype + protocol + years_of_education + hours_of_sleep_baseline +
                                                 protocol*session_number + protocol*genotype + session_number*genotype + session_number*protocol*genotype +(1|id),data=data_SHQ_1,  na.action = na.exclude)
...
lmer_SHQ_duration_EASY_levels <- lmer(log10(wayfinding_duration)  ~ as.factor(session_number) + age + sex + genotype + protocol + years_of_education + hours_of_sleep_baseline +
                                        protocol*session_number + protocol*genotype + session_number*genotype + session_number*protocol*genotype +(1|id), data=wayfinding_EASY,  na.action = na.exclude)

# two figures - MN and SD --> slide 14
# SD vs MN for e4+ and e4- --> slide 13

duration_easy <- anova (lmer_SHQ_duration_EASY_levels)
write.csv(duration_easy, 'C:\\Users\\adria\\Desktop\\duration_easy.csv')

duration_easy_eta <- eta_sq(lmer_SHQ_duration_EASY_levels)
write.csv(duration_easy_eta, 'C:\\Users\\adria\\Desktop\\duration_easy_eta.csv')
...
lms_SHQ_duration_EASY_levels <- lsmeans(lmer_SHQ_duration_EASY_levels, ~ session_number*genotype*protocol) # plot main effect of: genotype, protocol
...
lmer_SHQ_duration_HARD_levels <- lmer(log10(wayfinding_duration)  ~ as.factor(session_number) + age + sex + genotype + protocol + years_of_education + hours_of_sleep_baseline +
                                        protocol*session_number + protocol*genotype + session_number*genotype + session_number*protocol*genotype +(1|id), data=wayfinding_HARD,  na.action = na.exclude)

# two figures - MN and SD --> slide 14
# SD vs MN for e4+ and e4- --> slide 13

duration_hard <- anova (lmer_SHQ_duration_HARD_levels)
write.csv(duration_hard, 'C:\\Users\\adria\\Desktop\\duration_hard.csv')

duration_hard_eta <- eta_sq(lmer_SHQ_duration_HARD_levels)
write.csv(duration_hard_eta, 'C:\\Users\\adria\\Desktop\\duration_hard_eta.csv')
...
lms_SHQ_duration_HARD_levels <- lsmeans(lmer_SHQ_duration_HARD_levels, ~ session_number*genotype*protocol) # plot main effect of: genotype, protocol
